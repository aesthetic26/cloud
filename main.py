...
from helper import *
from config import API_ID, API_HASH, BOT_TOKEN, TG_CHANNEL, OWNER_ID
import database
...
async def require_auth(m: Message) -> bool:
    """Gate for work commands. Returns True if the sender is allowed to proceed.
    Sends a rejection message and returns False otherwise."""
    user_id = m.from_user.id if m.from_user else None
    if user_id is None or not await database.is_authorized(user_id):
        await m.reply_text(
...
async def start_command(bot: Client, message: Message):
    try:
        await database.record_broadcast_user(message.chat.id)
    except Exception:
        pass
    ...
    # Fetch the bot's own username dynamically so branding always points to itself
    me = await bot.get_me()
    ...
    caption = (
        "🌟 Welcome Boss🦁! 🌟\n\n"
        f"➽ I am Powerful DRM Uploader Bot 📥\n\n➽ 𝐔𝐬𝐞 /help for use this Bot.\n\n➽ **ᴊᴏɪɴ ᴏᴜʀ <a href='{TG_CHANNEL}'>__TG Channel__</a>**\n➽ **Add me in <a href='https://t.me/{bot_username}?startchannel=true'>__Your Channel__</a>**\n➽ **Add me in <a href='https://t.me/{bot_username}?startgroup=true'>__Your Group__</a>**\n\n<pre><code> 𝐌𝐚𝐝𝐞 𝐁𝐲 : 『ᴀᴅᴍɪɴ』 🦁</code></pre>"
    )

    # Inline keyboard for start command, built with the bot's own username
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(text="📞 Contact", url=f"https://t.me/{bot_username}"),
                InlineKeyboardButton(text="🛠️ Help", url=f"https://t.me/{bot_username}"),
            ],
        ]
    )
...
async def txt_handler(client: Client, m: Message):
    await bot.send_message(
        ...
        text=(
            ...
            "📥 /drm     - Extract .txt file (DRM decrypt)\n"
            "📄 /doc     - PDF/Image/Audio downloader\n"
            "🎬 /y2t     - YouTube playlist to .txt\n"
            "🎵 /ytm     - YouTube link/.txt to .mp3\n"
            "📝 /t2t     - Text to .txt convert\n"
            "🧹 /e2t     - Alphabetical sort\n"
            "🏷️ /title   - Clean title symbols\n"
            "🍪 /cookies - Update YT cookies\n"
            "🆔 /id      - Your Telegram ID\n"
            "ℹ️ /info    - Your Telegram info\n"
            "📋 /logs    - View bot logs\n"
            "🛑 /stop    - Stop running task (restarts)\n"
            "♻️ /reset   - Clear stuck state (no restart)\n"
            "━━━━━━━━━━━━━━━━━━━━━━\n"
    ...
    try:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            try:
                ...
                if not result:
                    await message.reply_text(
                        "🚨 : Could not read this playlist/video. Please check the link and try again."
                    )
                    return
        ...
        # Extract the YouTube links first, so we don't prompt for a filename
        # if the playlist turns out to have nothing usable in it.
        videos = []
        skipped = 0
        if 'entries' in result:
            for entry in result['entries']:
                ...
                if not url:
                    skipped += 1
                    continue
                video_title = entry.get('title') or 'No title'
                videos.append(f"{video_title}: {url}")
        else:
            url = result.get('url')
            if url:
                video_title = result.get('title') or 'No title'
                videos.append(f"{video_title}: {url}")

        if not videos:
            await message.reply_text(
                "🚨 : No accessible videos found in this playlist/video "
                "(they may all be private, deleted, or region-locked)."
            )
            return
        ...
        skipped_note = f"\n⚠️ Skipped {skipped} unavailable video(s)." if skipped else ""

        # Send the generated text file to the user with a pretty caption
        await message.reply_document(
            document=txt_file,
            caption=f'<a href="{youtube_link}">__**Click Here to open Playlist**__</a>\n<pre><code>{custom_file_name}.txt</code></pre>{skipped_note}\n'
        )
...
async def youtube_to_mp3(client, message: Message):
    if not await require_auth(message):
        return
    editable = await message.reply_text(
        "**🎵Send YouTube link(s) to download as `.mp3`.**\n"
        "Send single or multiple links (one per line), or a `.txt` file of links.\n"
    )
    ...
    try:
        ...
        if input_message.document:
            doc_path = await input_message.download()
            ...
            for line in input_message.text.splitlines():
                line = line.strip()
                if line.startswith("http://") or line.startswith("https://"):
                    links.append(line)
        ...
        if not links:
            await message.reply_text(
                "🚨 : Please send a valid YouTube link or a `.txt` file of links."
            )
            return

        failed = 0
        for link in links:
            status = await message.reply_text(f"⬇️ Downloading audio...\n{link}")
            audio_name = f"ytm_{int(time.time())}_{random.randint(1000,9999)}"
            audio_path = f"{audio_name}.mp3"
            try:
                cmd = (
                    f"yt-dlp -x --audio-format mp3 --cookies youtube_cookies.txt "
                    f"--retries 5 --sleep-requests 2 --extractor-retries 3 "
                    f"-o '{audio_name}.%(ext)s' '{link}'"
                )
                os.system(cmd)

                if not os.path.exists(audio_path):
                    failed += 1
                    await status.edit(f"🚨 : Could not download audio for\n{link}")
                    continue

                await message.reply_audio(
                    audio_path,
                    caption=f"🎵 Downloaded via /ytm\n{link}",
                )
                await status.delete()
            except Exception as e:
                failed += 1
                await status.edit(f"🚨 : Failed for {link}\n{str(e)}")
            finally:
        ...
        await message.reply_text(
            f"✅ **Done.** {len(links) - failed}/{len(links)} audio file(s) sent."
        )

    except asyncio.TimeoutError:
        await editable.delete(True)
        await message.reply_text("🚨 : Timed out waiting for a link.")
    except Exception as e:
...
def cleanup_partial_downloads():
    junk_exts = (
        ".mp4", ".mkv", ".webm", ".ts", ".m4a", ".mp3", ".wav",
        ".pdf", ".zip", ".jpg", ".jpeg", ".png", ".html",
        ".part", ".ytdl", ".aria2", ".tmp"
    )
    protected = {"youtube_cookies.txt", "cookies.txt"}
    for f in os.listdir("."):
        if f in protected:
            continue
        if os.path.isfile(f) and f.lower().endswith(junk_exts):
            try:
                os.remove(f)
            except Exception:
                pass
...
async def reset_handler(_, m):
    if not await require_auth(m):
        return
    global failed_links, processing_request
    cleanup_partial_downloads()
    failed_links.clear()
    processing_request = False
    await m.reply_text("<pre><code>⌈✨『𝚁𝙴𝚂𝙴𝚃』✨⌋</code></pre>", True)


async def require_owner(m: Message) -> bool:
    user_id = m.from_user.id if m.from_user else None
...
async def addauth_handler(_, m: Message):
    if not await require_owner(m):
        return
    if len(m.command) < 2:
        await m.reply_text("Usage: `/addauth <user_id>`")
        return
    try:
        ...
        return
    ok = await database.add_authorized_user(target_id)
    if ok:
        await m.reply_text(f"✅ User `{target_id}` added to authorized list.")
    else:
        await m.reply_text("🚫 Could not add user — MongoDB is not configured (MONGO_URI missing).")


@bot.on_message(filters.command("rmauth"))
async def rmauth_handler(_, m: Message):
    if not await require_owner(m):
        return
    if len(m.command) < 2:
        await m.reply_text("Usage: `/rmauth <user_id>`")
        return
    try:
        target_id = int(m.command[1])
    except ValueError:
        await m.reply_text("🚫 Invalid user ID.")
        return
    removed = await database.remove_authorized_user(target_id)
    if removed:
        await m.reply_text(f"✅ User `{target_id}` removed from authorized list.")
    else:
        await m.reply_text(f"ℹ️ User `{target_id}` was not in the authorized list.")
...
async def users_handler(_, m: Message):
    ...
    if not users:
        await m.reply_text("ℹ️ No authorized users yet (besides the owner).")
        return
...
async def broadcast_handler(_, m: Message):
    ...
    if len(m.command) < 2 and not m.reply_to_message:
        await m.reply_text("Usage: `/broadcast <message>` (or reply to a message with /broadcast)")
        return
    text = m.reply_to_message.text if m.reply_to_message and m.reply_to_message.text else " ".join(m.command[1:])
    recipients = await database.get_broadcast_users()
    if not recipients:
        await m.reply_text("ℹ️ No known users to broadcast to yet.")
        return
    sent, failed = 0, 0
    status = await m.reply_text(f"📢 Broadcasting to {len(recipients)} user(s)...")
...
async def broadusers_handler(_, m: Message):
    if not await require_owner(m):
        return
    recipients = await database.get_broadcast_users()
    if not recipients:
        await m.reply_text("ℹ️ No broadcast users recorded yet.")
        return
    text = "💌 **Broadcast Users:**\n" + "\n".join(f"• `{c}`" for c in recipients)
    await m.reply_text(text)
...
async def account_login(bot: Client, m: Message):
 ...
 #   await input4.delete(True)
    ...
    batch_msg = await m.reply_text(f"<pre><code>🎯**Target Batch : {b_name}**</code></pre>\n")
    try:
        await batch_msg.pin(disable_notification=True)
    except Exception:
        pass
